  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', userSelect: 'none', WebkitUserSelect: 'none', gap: spacing.sm }}>
      <div style={{ display: 'grid' }}>
        ...
        <span style={{ gridArea: '1/1', ...typography.label.sm, color: 'black', userSelect: 'none', WebkitUserSelect: 'none', opacity: showValue ? 1 : 0 }}>{Math.round(value * 100)}</span>
      </div>
      <div onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} style={{ cursor: 'pointer', position: 'relative', width: `${width}px`, height: '2px', borderRadius: borderRadius.md, backgroundColor: colors.bg.app, overflow: 'visible' }} ref={trackRef}>
        <div style={{ position: 'absolute', backgroundColor: colors.bg.canvas, width: `${value * width}px`, height: '2px', left: 0, borderRadius: borderRadius.md, overflow: 'visible' }} />
        <motion.div transition={{ type: 'spring', stiffness: 200, damping: 35 }} animate={{ width: showValue ? 24 : 16, height: showValue ? 24 : 16 }} style={{ display: 'flex', justifyContent: 'center', left: value * width - halfCircleSize, top: '50%', transform: 'translateY(-50%)', position: 'absolute', alignItems: 'center', borderRadius: '50%', backgroundColor: 'transparent', border: '1px solid blue' }}>
